  ...
  <li v-for="item of items" :key="item.id" class="mb-auto grid text-center">
    ...
    >
      <NuxtLink :to="`/categories/${item.slug}`" class="flex items-center gap-2">
        ...
        <span
          class="flex rounded-lg border border-transparent bg-linear-to-r bg-clip-text py-2 text-center text-transparent transition-colors duration-200 ease-in-out hover:from-blue-600 hover:to-purple-700 dark:from-gray-50 dark:to-gray-50 dark:hover:from-blue-300 dark:hover:to-purple-400"
      ...
      <div class="accordion-body grid grid-rows-[0fr] transition-all duration-300 ease-in-out">
        <ul class="border-muted-foreground min-h-0 overflow-hidden border-l pl-4">
    ...
    <NuxtLink v-else :to="`/categories/${item.slug}`" class="flex items-center gap-2">
      <Icon v-if="item.icon" :name="item.icon" class="size-5 text-sky-600" />
      <span
        class="flex rounded-lg border border-transparent bg-linear-to-r bg-clip-text py-2 text-center text-sm font-medium text-transparent transition-colors duration-200 ease-in-out hover:from-blue-600 hover:to-purple-700 dark:from-gray-50 dark:to-gray-50 dark:hover:from-blue-300 dark:hover:to-purple-400"
      >
        {{ item.name }}
      </span>
      <span
